...
def make_general_stats(data, headers):
    """Prepares data and headers for the general table section."""
    gen_data = {}
    gen_headers = {}
    for sample in data:
        for metric in data[sample]:
            ...
            if not ("exclude" in headers[metric] and headers[metric]["exclude"]):
                ...
                m_id = "gen table_" + metric
                if sample not in gen_data:
                    gen_data[sample] = {}
                gen_data[sample][m_id] = data[sample][metric]
                gen_headers[m_id] = headers[metric].copy()

    return gen_data, clean_headers(order_headers(gen_headers))


def make_own_table_plot(data, headers, table_config):
    """Creates plot for the non-general section."""
    own_data = {}
    own_headers = {}
    for sample in data:
        for metric in data[sample]:
            ...
            if not ("exclude_own" in headers[metric] and headers[metric]["exclude_own"]):
                # Make exclusive metric ID.
                m_id = "own table_" + metric
                if sample not in own_data:
                    own_data[sample] = {}
                own_data[sample][m_id] = data[sample][metric]
                own_headers[m_id] = headers[metric].copy()
                if "hidden_own" in own_headers[m_id]:
                    own_headers[m_id]["hidden"] = own_headers[m_id]["hidden_own"]

    return table.plot(own_data, clean_headers(order_headers(own_headers)), table_config)
